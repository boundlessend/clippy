...
- **факты по персонажу** - у каждого персонажа свои факты. У Clippy - ~700 реплик в его характере; свой персонаж показывает факты, только если положить ему `tips.json`, иначе ничего не всплывает.
...
- **обновления** - раз в сутки тихо проверяется свежий релиз на GitHub; вручную - «Проверить обновления…» в меню приложения или кнопкой в настройках.
...
2. Откройте образ и перетащите `Clippy Mac.app` в `Applications`.
3. Приложение не подписано Developer ID, поэтому macOS блокирует первый запуск. Обход (один раз, любой способ): **Системные настройки -> Конфиденциальность и безопасность -> «Открыть всё равно»** (кнопка появляется после первой попытки запуска); либо в Терминале `xattr -dr com.apple.quarantine "/Applications/Clippy Mac.app"`; на старых macOS также работает правый клик по `.app` -> «Открыть».

### из исходников

Собрать `.dmg` самому (для нарядного установщика нужен `brew install create-dmg`, без него соберётся простой dmg):
...
Или запустить прямо из дерева проекта:

```bash
swift run
```

Требуется macOS 14+ и Xcode 15+ (Swift 5.9+); собирается на GitHub Actions под macOS 14.

## Источники контента

Источник выбирается в настройках, там же поля провайдеров (ключ Claude хранится в Keychain, а не открытым текстом).

- **Локальные советы** - из коробки: ~700 реплик скрепыша, категории переключаются в настройках.
- **Ollama** - запущенный `ollama serve` и модель; адрес и модель задаются в настройках.
- **Claude** - ключ API вставляется в настройках (или `ANTHROPIC_API_KEY`).
- **RSS** - адрес ленты в настройках.
...
- спрайты, тайминги анимаций и звуки взяты из [ClippyJS](https://github.com/smore-inc/clippy.js) (MIT); персонажи происходят из **Microsoft Agent** (Clippy/«Clippit», Merlin, Genie, Links, Rover - Microsoft; Bonzi - персонаж BonziBuddy, Bonzi Software)
