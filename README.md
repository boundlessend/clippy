...
3. Приложение не подписано Developer ID, поэтому при первом запуске его блокирует Gatekeeper. Обход (один раз): **правый клик по `ClippyMac.app` -> «Открыть» -> «Открыть»**; дальше запускается обычным двойным кликом. Альтернатива: Системные настройки -> Конфиденциальность и безопасность -> «Всё равно открыть».
