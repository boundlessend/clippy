...
clippy-mac/
  ...
  animations/
    clippy-wave-inbetween-poc/     # POC AI in-between кадров для Wave, не подключен к runtime
  Sources/ClippyMac/
...
- **плавность анимаций через in-between кадры.** текущий runtime играет sprite
  frames из `agent.json` и `map.png`; простое уменьшение `duration` ускоряет
  движение, но не добавляет плавности. Проверен POC для `Wave`:
  `animations/clippy-wave-inbetween-poc/`. Подход: взять соседние кадры,
  сгенерировать single-pose midpoint на chroma-key фоне, удалить фон, привести к
  `RGBA 124x93`, затем в будущем вставить midpoint в expanded sprite sheet и
  заменить `100ms` на `50ms + 50ms`. Нужно доделать quality pass: генерировать по
  одной паре, уменьшить style drift, убрать green fringe, проверить на dock-scale,
  затем только после подтверждения менять runtime assets.
...
- облачко привязано к точному rect иконки дока через Accessibility (`dockIconRect`);
  системный запрос доступа показывается один раз, без доступа - фолбэк на курсор
...
(анимация в доке + факт по клику у дока), динамическое облачко - готовы. Весь
бэклог фич закрыт. Открытая задача на потом: отдельные факты под каждого персонажа
(сейчас факты только про Clippy) и отдельный разбор плавности анимаций через
AI-generated in-between кадры. Подпись Developer ID / нотаризация и разбор ассетов
Microsoft делать не будем (проект «только для себя»).
