...
**берём (дёшево, в характере):**
- меню дока богаче: «Показать жест» (проиграть случайный жест по требованию, а не
  только при факте), «Случайный персонаж» (перебор без окна настроек), подменю
  «Персонаж» (выбор прямо из right-click)

**под вопросом (оценить, скорее нет):**
