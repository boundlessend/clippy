...
- `branching`/вероятностные переходы анимаций - потом, если idle мёртвый
- звуки скрепыша - нет
- другие персонажи (Merlin, Rover, Links) - только Clippy
- отдельное окно настроек с UI - на старте хватает меню трея
- цепочка фолбэков между провайдерами контента - по запросу
- подпись/нотаризация/раздача - не в этой итерации

## git-стратегия

- локальный репозиторий, **пока ничего не публикуем** (нет remote, нет push)
...
## бэклог (из Cosmo/Clippy, отложено)

- **9** кастомные агенты из папки `.agent` + «Show in Finder» + «Reload»
- **10** конвертер `.acs` (any Microsoft Agent персонаж), по мотивам `agent-convert.sh`
- **11** MoveTo / GestureAt - скрепыш ходит по экрану и жестикулирует в точку

## следующий шаг

наполнение `tips.json` продумываем отдельно. основной цикл MVP (P0-P5) готов;
идёт P6 - оживление скрепыша.
