...
| распространение | публичные релизы на GitHub | ad-hoc подпись без нотаризации; тег `vX.Y.Z` -> CI собирает `.dmg` (см. RELEASING.md) |
...
- `RSSProvider`: случайный заголовок из первых 10 записей ленты (RSS 2.0 `<item>`
  или Atom `<entry>`, CDATA-заголовки поддерживаются), обрезка длинного по слову
...
- **лицензия ассетов:** спрайты Clippy - это Microsoft Agent (код ClippyJS под
  MIT, но сами картинки - IP Microsoft). Раздача в публичных релизах - серая зона;
  риск осознанный, README (Credits) прямо оговаривает «личное некоммерческое использование»

## git-стратегия

- проект опубликован: `github.com/boundlessend/clippy` (CI, релизы с DMG, лицензия)
- **релизы:** версия из git-тега (единый источник). тег `vX.Y.Z` ->
  `.github/workflows/release.yml` гоняет selftest, собирает `.dmg` на macOS-раннере и
  публикует GitHub Release с файлом + инструкцией по Gatekeeper. без Sparkle: приложение
  само раз в сутки проверяет свежий релиз через GitHub API (`UpdateCheck.swift`, есть и
  ручная проверка из меню), новый DMG пользователь скачивает сам. детали - в `RELEASING.md`
...
`tips.json` (~700 реплик в его характере, 7 категорий). Другой персонаж читает
...
- ~~наполнить `tips.json`~~ сделано: ~700 реплик в стиле «Скрепыш с характером»
  (персона/самоирония, ретро-техника 90-х, история вычислений, интернет-факты,
  наука/природа, советы/забота, юмор). Коротко, без упоминаний изменяемых настроек
...
**не берём:**
- Homebrew-cask / `distribute.sh` - cask с ad-hoc подписью без нотаризации неудобен
  (Gatekeeper всё равно блокирует установку); наш канал - DMG со страницы Releases
...
MVP, UI (окно настроек/About/версия), ~700 локальных фактов по категориям, поля
