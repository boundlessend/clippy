...
**исправлено (коммиты `1c0b0a1`…`f262572`):**
...
- ключ Claude пишется в Keychain на каждый символ ввода (значение всегда корректно,
  статус проверяется) - дебаунс/commit-on-submit не делали
- точная привязка облачка к rect иконки дока при autohide/magnification - через
  Accessibility (нужно разрешение); сейчас якорь по курсору покрывает клик-сценарий
- ATS оставлен строгим: внешний RSS по http не заработает (нужен https) - без
  небезопасного исключения
- retain-cycle `delegate ↔ settingsWindow ↔ ClippyControls` - безвреден (AppDelegate
  живёт всё время работы), намеренно не ломаем
...
(сейчас факты только про Clippy). Подпись Developer ID / нотаризация и разбор
ассетов Microsoft делать не будем (проект «только для себя»).
