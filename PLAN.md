...
**8. Автозапуск.** через `LaunchAgent` (`LoginItem.swift`): пишем plist в
`~/Library/LaunchAgents/com.clippymac.agent.plist` с `ProgramArguments` =
`Bundle.main.executablePath` и `RunAtLoad`, затем `launchctl load`. `SMAppService`
не используем - для незаверенного .app он капризен, а LaunchAgent работает без подписи.
- **важно:** тумблер включать из установленного в `/Applications` приложения, а не из
  `swift run` (иначе plist укажет на временный бинарь сборки)
...
clippy-mac/
  ...
  Sources/ClippyMac/
    ...
    LoginItem.swift                # автозапуск (LaunchAgent plist + launchctl)
...
- парсинг ClippyJS `agent.js`: `branching` сложнее линейного проигрывания;
  на старте упрощаем
- автозапуск: `SMAppService` для незаверенного .app капризен, поэтому используем
  `LaunchAgent` plist (`LoginItem.swift`), он работает без подписи
...
- **кормление файлами (dockitty).** файл, брошенный на иконку дока
  (`application(_:open:)` + типы документов `public.item` в Info.plist), вызывает
  реакцию персонажа и облачко «Ням!». Режим корзины - тумблер «Кормление файлом
  отправляет его в Корзину» (по умолчанию **включён**, без подтверждения): файл уходит
  в Корзину (`NSWorkspace.recycle`, обратимо), а не удаляется навсегда
...
### следующее: редизайн настроек + приветствие (макет одобрен)

текущее окно настроек (`SwiftUI Form`) выглядит невзрачно. Одобрен HTML-прототип нового
UI (`design/settings-onboarding-mock.html`): современное минималистичное окно с
группами-карточками (Основное / Персонаж / Факты / Поведение), нативный SF, янтарный
акцент под жёлтую скрепку, спрайты персонажей, тёмная тема; плюс приветственное окно
первого запуска из 6 шагов (интро -> доступ Accessibility -> персонаж -> источник ->
базовые тумблеры -> готово), переоткрываемое из настроек. TODO: перенести макет в
SwiftUI (`SettingsRootView` + новый онбординг), флаг «первый запуск» в `AppSettings`.
...
**активная задача:** редизайн окна настроек + приветственное окно первого запуска по
одобренному макету `design/settings-onboarding-mock.html` (перенос в SwiftUI).

**на потом:** отдельные факты под каждого персонажа (сейчас факты только про Clippy).
Подпись Developer ID / нотаризация и разбор ассетов Microsoft делать не будем (проект
«только для себя»).
