...
- **анимированная иконка в доке** - пока приложение запущено, выбранный персонаж анимируется прямо в доке (idle-жесты, branching-кадры).
- **факт по клику** - левый клик по иконке в доке показывает новый факт в облачке рядом с ней.
- **облачко с учётом дока** - баллон встаёт у иконки, а хвостик смотрит в сторону дока, где бы док ни стоял (внизу / слева / справа).
- **меню дока** - правый клик по иконке: Показать факт / Настройки… / О программе.
- **персонажи** - Clippy и ещё пятеро в комплекте (Merlin, Genie, Bonzi, Links, Rover), плюс любой свой персонаж из папки `Agents` (подпапка с `agent.json` + `map.png`), переключение в настройках. Многослойные спрайты композитятся, поэтому полноценные персонажи Microsoft Agent рисуются правильно. Факты пока только про Clippy; другим персонажам показываются лишь общие факты (свои реплики - в планах).
...
- **меню-бар (опционально)** - опциональная иконка в трее с тем же меню, по умолчанию выключена, на случай скрытого дока.
