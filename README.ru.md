...
- **персонажи** - встроенный скрепыш плюс свои персонажи из папки `Agents` (каждый - подпапка с `agent.json` + `map.png`), переключение в настройках. Многослойные спрайты композитятся, поэтому полноценные персонажи Microsoft Agent рисуются правильно.
